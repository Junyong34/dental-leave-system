...
#### 섹션 2 상세 동작 (업데이트)
- 이름 수정은 인라인 입력이 아닌 **다이얼로그**에서만 수행
  - `수정` 버튼 클릭 시 이름 수정 다이얼로그 오픈
  - 빈 값 저장 차단
  - 전체 직원(재직/퇴사 포함) 기준 이름 중복 차단
  - 변경 없음 저장 시 API 호출 없이 종료
- 이름 저장/상태 변경 중에는 동일 직원 행의 액션 버튼을 비활성화하여 중복 요청 방지
- 모바일(`<=768px`)에서는 직원 관리 카드 상단의 `여러 명 추가`, `직원 추가` 버튼 크기를 축소해 가독성 개선
...
- ✅ 설정 페이지 야간 근무 관리 섹션 (`/settings/night-shift`)
  - 직원 관리 (추가/수정/삭제/대량추가)
    - 직원 이름 수정 다이얼로그 적용 (인라인 편집 제거)
    - 이름 검증: 공백/중복 차단
    - 모바일 버튼 사이즈 최적화 (`여러 명 추가`, `직원 추가`)
  - 야간 요일 설정
  - 야간 근무 기록 관리
